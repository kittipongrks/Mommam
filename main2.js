...
addForm.addEventListener("submit", async (e) => {
    ...
    if (addForm.checkValidity() === false) {
        ...
        const data = await fetch("action_admin.php", {
            method: "POST",
            body: formData
        })
...
const fetchAllUsers = async () => {
    const data = await fetch("action_admin.php?read=1", {
        method: "GET"
    })
    const response = await data.text();
...
const editUser = async (UserID) => {
    const data = await fetch(`action_admin.php?edit=1&id=${UserID}`, {
        method: "GET"
    })
...
const deleteUser = async (UserID) => {
    const data = await fetch(`action_admin.php?delete=1&id=${UserID}`, {
        method: "GET"
    })
    const response = await data.text();
    showAlert.innerHTML = response;
